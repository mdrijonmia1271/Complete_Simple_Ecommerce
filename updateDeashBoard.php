...
if(isset($_POST['submit'])){
  ...
  if(empty($admin_name)){
    $emtAdminName = 'Fill Up this field';
  }

  if(!empty($category_name_id) && !empty($admin_name) && !empty($sub_category_name_id) && !empty($color_name_id) && !empty($size_name_id) && !empty($packet_name_id) && !empty($quantity)){
    $statement = $pdo ->prepare('UPDATE deash_board SET category_name_id = :category_name_id, admin_name = :admin_name, sub_category_name_id = :sub_category_name_id, color_name_id = :color_name_id, size_name_id = :size_name_id, packet_name_id = :packet_name_id, quantity = :quantity, description = :description, status = :status WHERE id = :id');
           ...
           <form action="" method="POST" enctype="multipart/form-data">
            <h5>Add Deash Board Products</h5>
              ...
              <tr>
                  <th>Admin Name:</th>
                  <td>
                    <input type="text" name="admin_name" placeholder="Enter Your Name" value="<?php echo $product ['admin_name'] ?>">
                    <br>
                    <?php if(isset($_POST['submit'])){echo "<span class='text-danger'>".$emtAdminName."</span>";} ?>
                  </td>
                </tr>
